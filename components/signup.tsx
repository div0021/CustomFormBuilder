...
const Signup:React.FC<SignupProps> = ({handleAuth}) => {
    ...
    const onSubmit:SubmitHandler<SignupSchemaType> = async (values) => {
       ...
       if(res?.data?.errorMessage){
        toast({title:"Something went wrong!",description:res.data.errorMessage,
        variant:"destructive"
    });
          return;
       }
    ...
    return (
        <div className="w-full h-full bg-black/90">
            <div className="w-full h-[100dvh] flex justify-center items-center text-white">
                <div className="overflow-hidden bg-purple-500 rounded-xl p-1 relative before:absolute before:bg-gray-500 before:w-[60rem] before:h-[60rem] before:rounded-full before:top-1/2 before:left-1/2 
                before:bg-gradient-to-r from-[#d896ff] to-[#660066] before:animate-mySpin before:z-10 min-w-[350px] max-w-[530px]">
                    ...
                    <div>
                        ...
                        <Input label="Name" icon={User2} isPassword={false} placeholder="Enter Your name" type="text" register={form.register} disabled={loading} id="name" authType="signup" />
                        ...
                        {form.formState.errors.name && (
                            ...
                            <Input label="Email" icon={Mail} isPassword={false} placeholder="Enter Your email" type="text" register={form.register} disabled={loading} id="email" authType="signup"/>
                            ...
                            <Input label="Password" type="password" isPassword placeholder="Enter your password" icon={Lock} register={form.register} disabled={loading} id="password" authType="signup"/>
                    ...
                    <div className="mt-5 w-full flex justify-center items-center">
                            ...
                            <div>
                                <p className="text-center text-sm">Already have an <span className="underline text-[#d896ff] cursor-pointer" onClick={() => handleAuth("login")}>account</span> ?</p>
                            </div>
                    </div>
                </div>
            </div>
